...
public class SkillDatabase {
    ...
    protected static HashMap<Player, List<Perk>> ownedPerks = new HashMap<>();
    protected static HashMap<Player, List<Perk>> activePerks = new HashMap<>();
    protected static HashMap<Player, Integer> skillLevel = new HashMap<>();
    protected static HashMap<Player, Integer> butcherLevel = new HashMap<>();
    protected static HashMap<Player, Integer> cavemanLevel = new HashMap<>();
    protected static HashMap<Player, Integer> naturalistLevel = new HashMap<>();
    protected static HashMap<Player, Integer> huntsmanLevel = new HashMap<>();
    protected static HashMap<Player, Integer> harvesterLevel = new HashMap<>();
    protected static HashMap<Player, Integer> intellectualLevel = new HashMap<>();
    protected static HashMap<Player, Integer> explorerLevel = new HashMap<>();
    protected static HashMap<Player, Integer> survivalistLevel = new HashMap<>();
    protected static HashMap<Player, BigDecimal> skillProgress = new HashMap<>();
    protected static HashMap<Player, Integer> skillPoints = new HashMap<>();
    protected static HashMap<Player, SkillType> mainSkills = new HashMap<>();
    ...
    public static void clearDatabase() {
        ...
        try {
            ...
            sendUnsafeQuery("drop table huntsmanlevel");
    ...
    private static List<Perk> getActivePerks(Player p) {
        ...
        ResultSet result = sendResultQuery("select perk from activePerks where uuid = '" + p.getUniqueId() + "'");
        try {
    ...
    public static List<Perk> getActivePerks(String uuid) {
        List<Perk> perks = new ArrayList<>();
        ResultSet result = sendResultQuery("select perk from activePerks where uuid = '" + uuid + "'");
        try {
            while (result != null && result.next()) {
                perks.add(Perk.valueOf(result.getString(1)));
            }
    ...
    private static List<Perk> getOwnedPerks(Player p) {
        List<Perk> perks = new ArrayList<>();
        ResultSet result = sendResultQuery("select perk from ownedPerks where uuid = '" + p.getUniqueId() + "'");
        try {
            while (result != null && result.next()) {
                try {
                    perks.add(Perk.valueOf(result.getString(1)));
    ...
    private static String getData(Player p, String table) {
        try {
            ...
            if (result != null && result.next()) return result.getString(1);
        } catch (SQLException e) {
            Bukkit.getLogger().warning("Error loading " + table + " data for: " + p.getName());
            e.printStackTrace();
        }
        return (table.equals("userpoints") || table.equals("userprogress")) ? "0" : "1";
    }
    ...
    public static void save(Player player) {
        ...
        try {
            ...
            clearData(player); // still using sendQuery, but now in transaction
    ...
    public static boolean isNatural(Location location) {
        try {
            return !sendResultQuery("select * from locations where world = '" + location.getWorld().getName() + "' and x = " + location.getBlockX() + " and y = " + location.getBlockY() + " and z = " + location.getBlockZ()).next();
        } catch (SQLException e) {
            return true;
        }
    }
    ...
    public static void placedBlock(Location location) {
        if (mySQL) {
            ...
            sendQuery("insert or ignore into locations values ('" + location.getWorld().getName() + "'," + location.getBlockX() + "," + location.getBlockY() + "," + location.getBlockZ() + ")");
        }
    }

    public static void grownBlock(Location location) {
        sendQuery("delete from locations where world = '" + location.getWorld().getName() + "' and x = " + location.getBlockX() + " and y = " + location.getBlockY() + " and z = " + location.getBlockZ());
    }
    ...
    public static HashMap<Player, Integer> getSkill(SkillType skill) {
        switch(skill) {
            case BUTCHER: return butcherLevel;
            case CAVEMAN: return cavemanLevel;
            case EXPLORER: return explorerLevel;
            case HARVESTER: return harvesterLevel;
            case HUNTSMAN: return huntsmanLevel;
            case INTELLECTUAL: return intellectualLevel;
            case NATURALIST: return naturalistLevel;
            case SURVIVALIST: return survivalistLevel;
        }
        return null;
    }
    ...
    public static List<String> getSkillTop() {
        ...
        ResultSet result = sendResultQuery("select * from userlevels order by value desc");
        try {
            int i = 0;
            while (result != null && result.next() && i < 10) {
